...
import { getRoutingTableInfo } from './routing.js';
...
export class DHTNode {
    ...
    async start() {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.KEY_GENERATION_FAILED, `Failed to start DHT node: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error instanceof Error ? error : undefined });
        }
    }
    /**
     * Stop the DHT node.
    ...
    async bootstrap(peers) {
        ...
        if (connectedCount === 0 && bootstrapPeers.length > 0) {
            throw new DHTError(DHTErrorCode.BOOTSTRAP_FAILED, `Failed to connect to any bootstrap peers. Attempted ${bootstrapPeers.length} peer(s).`, {
                context: {
    ...
    async findPeer(peerId) {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.NOT_FOUND, `Peer not found: ${targetPeerId.toString()}`, { context: { peerId: targetPeerId.toString() } });
        }
        catch (error) {
            if (error instanceof DHTError) {
                throw error;
            }
            throw new DHTError(DHTErrorCode.NOT_FOUND, `Failed to find peer: ${error instanceof Error ? error.message : 'Unknown error'}`, {
    ...
    async put(key, value) {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.PUT_FAILED, `Failed to store value in DHT: ${error instanceof Error ? error.message : 'Unknown error'}`, {
                cause: error instanceof Error ? error : undefined,
    ...
    async get(key) {
        ...
        // Validate key
        if (!key || key.length === 0) {
            throw new DHTError(DHTErrorCode.INVALID_RECORD, 'Key cannot be empty', { context: { keyLength: key?.length ?? 0 } });
        }
        try {
            const dht = this.getDHTService();
            ...
            throw new DHTError(DHTErrorCode.NOT_FOUND, 'Key not found in DHT', { context: { keyLength: key.length } });
        }
        catch (error) {
            if (error instanceof DHTError) {
                throw error;
            ...
            if (errorMessage.toLowerCase().includes('not found') ||
                ...
                throw new DHTError(DHTErrorCode.NOT_FOUND, 'Key not found in DHT', {
                    cause: error instanceof Error ? error : undefined,
                    context: { keyLength: key.length },
                });
            }
            throw new DHTError(DHTErrorCode.NOT_FOUND, `Failed to retrieve value from DHT: ${errorMessage}`, {
                cause: error instanceof Error ? error : undefined,
                context: { keyLength: key.length },
            });
        }
    ...
    async *getClosestPeers(key) {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.NOT_FOUND, `Failed to get closest peers: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error instanceof Error ? error : undefined });
        }
    }
    /**
    ...
    async provide(key) {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.PROVIDE_FAILED, `Failed to provide content: ${error instanceof Error ? error.message : 'Unknown error'}`, {
                cause: error instanceof Error ? error : undefined,
    ...
    async *findProviders(key) {
        ...
        try {
            ...
            throw new DHTError(DHTErrorCode.NO_PROVIDERS, `Failed to find providers: ${error instanceof Error ? error.message : 'Unknown error'}`, {
                cause: error instanceof Error ? error : undefined,
                context: { cid: cid.toString() },
            });
        }
    ...
    validateKeyValue(key, value) {
        ...
        if (!value) {
            throw new DHTError(DHTErrorCode.INVALID_RECORD, 'Value cannot be null or undefined', { context: { keyLength: key.length } });
        }
    ...
    ensureStarted() {
        if (!this.started || !this.node) {
            throw new DHTError(DHTErrorCode.INVALID_CONFIG, 'DHT node is not started. Call start() first.');
        }
    }
    /**
    ...
    getDHTService() {
        ...
        if (!services?.dht) {
            throw new DHTError(DHTErrorCode.INVALID_CONFIG, 'DHT service is not available on this node');
        }
        return services.dht;
    }
    /**
     ...
     */
    sortPeersByDistance(peers, targetKey) {
    ...
    parseCID(key) {
        if (typeof key === 'string') {
            try {
                ...
                throw new DHTError(DHTErrorCode.INVALID_RECORD, `Invalid CID string: ${error instanceof Error ? error.message : 'Unknown error'}`, {
                    cause: error instanceof Error ? error : undefined,
                    context: { cid: key },
                });
            }
        ...
        if (!key || typeof key.toString !== 'function' || typeof key.multihash !== 'object') {
            throw new DHTError(DHTErrorCode.INVALID_RECORD, 'Invalid CID object', { context: { cid: String(key) } });
        }
        return key;
    }
}
