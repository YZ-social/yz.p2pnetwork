...
describe('Property 6: Put-Get Consistency', () => {
    ...
    it('empty key is rejected with INVALID_RECORD error for any value', async () => {
        await fc.assert(fc.asyncProperty(valueArbitrary, async (value) => {
            ...
            try {
                ...
                return error.code === DHTErrorCode.INVALID_RECORD;
            }
        }), { numRuns: 10 });
    });
    it('null value is rejected with INVALID_RECORD error for any key', async () => {
        await fc.assert(fc.asyncProperty(keyArbitrary, async (key) => {
            try {
                await node.put(key, null);
                return false; // Should have thrown
            }
            catch (error) {
                return error.code === DHTErrorCode.INVALID_RECORD;
    ...
    it('valid key-value pairs pass validation (put does not throw INVALID_RECORD)', async () => {
        await fc.assert(fc.asyncProperty(keyArbitrary, valueArbitrary, async (key, value) => {
            try {
...
//# sourceMappingURL=content.property.test.js.map