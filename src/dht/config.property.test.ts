...
const webrtcConfigArbitrary = fc.record({
  ...
  stunServers: fc.option(
    fc.array(fc.constantFrom('stun:stun.l.google.com:19302', 'stun:stun1.l.google.com:19302'), { minLength: 1, maxLength: 3 }),
    { nil: undefined }
  ),
 ...
 */
const listenAddressArbitrary = fc.constantFrom(
  '/ip4/0.0.0.0/tcp/0',
 ...
 */
const dhtNodeConfigArbitrary: fc.Arbitrary<DHTNodeConfig> = fc.record({
 ...
 */
function configsAreEquivalent(original: DHTNodeConfig, roundTripped: DHTNodeConfig): boolean {
  ...
  if (origKBucket !== roundTripped.kBucketSize) return false;

  const origAlpha = original.alpha ?? DEFAULT_CONFIG.alpha;
  if (origAlpha !== roundTripped.alpha) return false;

  const origRefresh = original.refreshInterval ?? DEFAULT_CONFIG.refreshInterval;
  if (origRefresh !== roundTripped.refreshInterval) return false;

  const origRecordExp = original.recordExpiration ?? DEFAULT_CONFIG.recordExpiration;
  if (origRecordExp !== roundTripped.recordExpiration) return false;

  const origProviderExp = original.providerExpiration ?? DEFAULT_CONFIG.providerExpiration;
  if (origProviderExp !== roundTripped.providerExpiration) return false;

  const origMaxConn = original.maxConnections ?? DEFAULT_CONFIG.maxConnections;
  if (origMaxConn !== roundTripped.maxConnections) return false;

  const origMinConn = original.minConnections ?? DEFAULT_CONFIG.minConnections;
  if (origMinConn !== roundTripped.minConnections) return false;

  // Optional string/boolean fields
  if (original.protocol !== roundTripped.protocol) return false;
  if (original.clientMode !== roundTripped.clientMode) return false;
  ...
  if (JSON.stringify(origBootstrap) !== JSON.stringify(rtBootstrap)) return false;

  // webrtc config
  if (JSON.stringify(original.webrtc) !== JSON.stringify(roundTripped.webrtc)) return false;

  // circuitRelay config
  if (JSON.stringify(original.circuitRelay) !== JSON.stringify(roundTripped.circuitRelay)) return false;
