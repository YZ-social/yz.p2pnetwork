...
describe('Property 4: Closest Peers Ordering', () => {
   ...
   */
  function sortByXorDistance(peerIds: Uint8Array[], targetKey: Uint8Array): Uint8Array[] {
  ...
  it('peers are sorted in ascending XOR distance order', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.uint8Array({ minLength: 32, maxLength: 32 }), { minLength: 2, maxLength: 20 }),
        (targetKey, peerIds) => {
  ...
  it('sorting is stable - same input produces same output order', () => {
    fc.assert(
      fc.property(
        fc.uint8Array({ minLength: 32, maxLength: 32 }),
        fc.array(fc.uint8Array({ minLength: 32, maxLength: 32 }), { minLength: 1, maxLength: 10 }),
        (targetKey, peerIds) => {
  ...
  it('closest peer has minimum XOR distance', () => {
    fc.assert(
      fc.property(
        fc.uint8Array({ minLength: 32, maxLength: 32 }),
        fc.array(fc.uint8Array({ minLength: 32, maxLength: 32 }), { minLength: 1, maxLength: 20 }),
        (targetKey, peerIds) => {
  ...
  it('sorting preserves all peers - no peers lost or duplicated', () => {
    fc.assert(
      fc.property(
        fc.uint8Array({ minLength: 32, maxLength: 32 }),
        fc.array(fc.uint8Array({ minLength: 32, maxLength: 32 }), { minLength: 0, maxLength: 15 }),
        (targetKey, peerIds) => {
          const sorted = sortByXorDistance(peerIds, targetKey);
          ...
          for (const peerId of peerIds) {
            const found = sorted.some(p => 
              p.length === peerId.length && p.every((b, i) => b === peerId[i])
            );
